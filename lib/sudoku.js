...
/**
 * How many candidate puzzles generateSudoku evaluates to match the felt
 * (qualitative) difficulty. The first candidate whose rating is within 1 of
 * the requested difficulty wins; otherwise the closest of the batch is kept.
 */
const RATING_CANDIDATES = 3;
...
/** All candidate digits that can legally be placed at board[r][c]. */
function candidatesAt(board, r, c) {
  const out = [];
  for (let n = 1; n <= 9; n++) {
    if (isValid(board, r, c, n)) out.push(n);
  }
  return out;
}

/** The 27 units (9 rows, 9 columns, 9 boxes) as arrays of [row, col] pairs. */
function allUnits() {
  const units = [];
  for (let i = 0; i < 9; i++) {
    units.push(Array.from({ length: 9 }, (_, j) => [i, j])); // row i
    units.push(Array.from({ length: 9 }, (_, j) => [j, i])); // column i
    const br = Math.floor(i / 3) * 3;
    const bc = (i % 3) * 3;
    units.push(
      Array.from({ length: 9 }, (_, j) => [br + Math.floor(j / 3), bc + (j % 3)]) // box i
    );
  }
  return units;
}
...
/**
 * Simulate a human solver using the two basic logical techniques and report
 * how far they get. The input puzzle is not modified.
 *
 * - naked single: an empty cell with exactly one legal candidate
 * - hidden single: a digit with exactly one possible cell inside a unit
 *
 * @param {number[][]} puzzle - 9x9 array (0 = empty)
 * @returns {{ nakedSingles: number, hiddenSingles: number, solvedByLogic: boolean }}
 */
export function analyzePuzzle(puzzle) {
  ...
  while (progress) {
    ...
    // Pass 1 — naked singles (the easiest technique)
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        if (board[r][c] !== 0) continue;
        const cands = candidatesAt(board, r, c);
        if (cands.length === 1) {
          board[r][c] = cands[0];
          nakedSingles++;
          progress = true;
    ...
    if (progress) continue; // exhaust naked singles before trying harder

    // Pass 2 — hidden singles (requires scanning whole units)
    for (const unit of units) {
      for (let n = 1; n <= 9; n++) {
        if (unit.some(([r, c]) => board[r][c] === n)) continue;
        const spots = unit.filter(([r, c]) => board[r][c] === 0 && isValid(board, r, c, n));
        if (spots.length === 1) {
          const [r, c] = spots[0];
          board[r][c] = n;
          hiddenSingles++;
  ...
  const solvedByLogic = board.every((row) => row.every((v) => v !== 0));
  return { nakedSingles, hiddenSingles, solvedByLogic };
}

/**
 * Qualitative difficulty rating of a puzzle on the same 1-10 scale as
 * generateSudoku's input.
 *
 * Combines the quantitative signal (number of empty cells — inverse of the
 * removal formula) with a qualitative adjustment from analyzePuzzle:
 *   - solvable with naked singles only  → feels easier   (-1)
 *   - needs hidden singles              → as expected     (0)
 *   - not solvable by basic logic       → feels harder   (+2)
 *
 * @param {number[][]} puzzle - 9x9 array (0 = empty)
 * @returns {number} integer rating in [1, 10]
 */
export function ratePuzzle(puzzle) {
  const empties = puzzle.flat().filter((v) => v === 0).length;
  const quantitative = 1 + (empties - 25) / 3.67;

  const { hiddenSingles, solvedByLogic } = analyzePuzzle(puzzle);
  let adjustment;
  if (!solvedByLogic) adjustment = 2;
  else if (hiddenSingles > 0) adjustment = 0;
  else adjustment = -1;

  return Math.max(1, Math.min(10, Math.round(quantitative + adjustment)));
}
 ...
 *
 * Each candidate puzzle is scored with ratePuzzle (qualitative heuristic);
 * up to RATING_CANDIDATES candidates are generated and the one whose felt
 * difficulty is closest to the request is returned, with its rating attached.
 *
 ...
 * @returns {{ puzzle: number[][], solution: number[][], rating: number }}
 */
export function generateSudoku(difficulty, seed) {
  ...
  for (let candidate = 0; candidate < RATING_CANDIDATES; candidate++) {
    let result = null;
    while (!result) {
      ...
      result = tryGenerate(difficulty, rng);
      if (!result) failures++;
    }

    const rating = ratePuzzle(result.puzzle);
    const gap = Math.abs(rating - difficulty);
    if (gap < bestGap) {
      best = { ...result, rating };
      bestGap = gap;
    }
    if (bestGap <= 1) break; // close enough — no need for more candidates
  }
 ...
 * @param {object} [options] - { font: 'helvetica'|'times'|'courier',
 *                               color: [r, g, b] for the digits }
 */
export function drawGridOnPDF(doc, board, x, y, size, options = {}) {
  const { font = 'helvetica', color = [30, 30, 30] } = options;
  const cell = size / 9;
...
/**
 * Draw one sudoku grid onto a Canvas 2D context (used for PNG export).
 * Mirrors drawGridOnPDF, with pixel units instead of mm.
 *
 * @param {CanvasRenderingContext2D} ctx
 * @param {number[][]} board  - 9x9 array (0 = empty)
 * @param {number} x          - left edge in px
 * @param {number} y          - top edge in px
 * @param {number} size       - grid width/height in px (square)
 */
export function drawGridOnCanvas(ctx, board, x, y, size) {
  const cell = size / 9;
  ...
  ctx.font = `bold ${Math.floor(cell * 0.55)}px Helvetica, Arial, sans-serif`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      if (board[r][c] !== 0) {
        ctx.fillText(String(board[r][c]), x + c * cell + cell / 2, y + r * cell + cell / 2 + 1);
      }
    }
  }
  ...
  for (let i = 0; i <= 9; i++) {
    const isBox = i % 3 === 0;
    ctx.lineWidth = isBox ? 3 : 1;
    ctx.strokeStyle = isBox ? '#000000' : '#a0a0a0';
    ctx.beginPath();
